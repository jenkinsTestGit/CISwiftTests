...
+ (UIImage *)expectedImageWithName:(NSString *)name
{
    ...
    //Customization for swift project with Pods!!!
    NSString *path = [self directoryForExpectedImages];
    path = [path substringToIndex:[path length]-26];
    //
    NSString *filePath = [path stringByAppendingPathComponent:fileName]; //path replace [self directoryForExpectedImages]
